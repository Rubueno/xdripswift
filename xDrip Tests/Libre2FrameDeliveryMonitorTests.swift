...
final class Libre2FrameAssemblerTests: XCTestCase {
    func testConsecutiveFramesCompleteWithoutWaitingForTimeout() {
        ...
        XCTAssertEqual(assembler.append(Data([0, 1]), arrival: start), .incomplete)
        XCTAssertEqual(assembler.append(Data([2, 3]), arrival: start), .incomplete)
        XCTAssertEqual(
            assembler.append(Data([4, 5]), arrival: start),
            .complete(frame: Data([0, 1, 2, 3, 4, 5]), assemblyDuration: 0)
        )

        XCTAssertEqual(assembler.append(Data([6, 7, 8]), arrival: start), .incomplete)
        XCTAssertEqual(
            assembler.append(Data([9, 10, 11]), arrival: start),
    ...
    func testPartialFrameIsDiscardedAfterAssemblyTimeout() {
        ...
        XCTAssertEqual(assembler.append(Data([0, 1, 2]), arrival: start), .incomplete)
        XCTAssertEqual(
            assembler.append(Data([3, 4, 5]), arrival: start.advanced(by: .seconds(4))),
            .incomplete
        )
    }
    ...
    func testOversizedFrameIsDiscardedAndNextFrameCanComplete() {
        ...
        XCTAssertEqual(
            assembler.append(Data([0, 1, 2, 3, 4, 5, 6]), arrival: start),
            .oversized(receivedByteCount: 7)
        )
        XCTAssertEqual(
            assembler.append(Data([7, 8, 9, 10, 11, 12]), arrival: start),
